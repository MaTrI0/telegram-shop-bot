...
# Выполнение функции после старта бота после запуска бота
async def on_startup(dp: Dispatcher):
    ...
    bot_logger.warning("Бот запускается...")
    print(colorama.Fore.LIGHTYELLOW_EX + "~~~~~ Бот запущен! ~~~~~")
    print(colorama.Fore.RESET)

    if len(get_admins()) == 0: print("***** Введите ID Администратора, так как их нет в setting.ini *****")
